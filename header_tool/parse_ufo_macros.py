...
def search_file(_header_file):
    ...
    class StackObjectMacro(StackObject):
        ...
        def search(self, _pending_macro, _classes, _scope):
            ...
            _pending_macro.macros.append(
                {"name": self.name, "args": [i.name for i in self.members]}
            )
    ...
    class StackObjectVariable(StackObject):
        ...
        def search(self, _pending_macro, _member_variables, _scope):
            if _pending_macro.pending:
                _member_variables.append(
                    [
                        _pending_macro.macros,
                        {
                            "name": self.name,
    ...
    for character in f.read():
        ...
        if character == "{":
            ...
            if words[-1].word == "namespace":
                stack.append(StackObjectNamespace(NAMESPACE, None))
            if words[-2].word == "namespace":
                stack.append(StackObjectNamespace(NAMESPACE, words[-1].word))
            ...
            if words[-2].word == ":":
                if words[-4].word == "class":
                    stack.append(
                        StackObjectClass(
                            CLASS, words[-3].word, words[-1].word, _header_file
                        )
                    )

            # Inheriting publicly
            if words[-2].word == "public":
                if words[-5].word == "class":
                    stack.append(
                        StackObjectClass(
                            CLASS, words[-4].word, words[-1].word, _header_file
                        )
                    )
        ...
        if character == "}" and (
            stack[-1].value == NAMESPACE
            or stack[-1].value == CLASS
            or stack[-1].value == FUNCTION_BODY
        ):
            words.append(Word(character, NONE))
            pop_stack(stack)
        ...
        if character == ";":
            ...
            if words[-2].word != "class" and (
                words[-1].word not in [")", "}", "{", "::", ":"]
                or words[-1].type_of_word == STRING
            ):
                print("Found variable")
                if words[-2].word == "=":
                    stack.append(
                        StackObjectVariable(
                            VARIABLE, words[-3].word, words[-4].word, words[-1].word
                        )
                    )
                    pop_stack(stack)
                    print(
                    ...
                    stack.append(
                        StackObjectVariable(
                            VARIABLE, words[-2].word, words[-3].word, None
                        )
                    )
                    pop_stack(stack)
        ...
        if character == "(":
            ...
            if res_class and not res_namespace:
                print(
                    "Found macro, function or function-call",
                    words[-1].word,
                    "inside class",
                    res_class.name,
                )
        ...
        if character == ")" and (
            stack[-1].value == MACRO_CALL or stack[-1].value == FUNCTION
        ):
            if word != "":
                words.append(Word(word, NONE))
            word = ""
...
def make_generated_file(_path, _classes):
    ...
    for cl in _classes:
        if cl.represents["header_file"] not in header_files:
            header_files.append(cl.represents["header_file"])
...
def start_search_directories_for_ufo_classes():
    arg_path = sys.argv[1]

    classes = []

    search_folders_for_ufo_classes(classes, arg_path, "")

    classes_as_dictionary = []

    for cl in classes:
        pprint.pprint(cl.macros)
        pprint.pprint(cl.represents)

        classes_as_dictionary.append({"macros": cl.macros, "class": cl.represents})

    make_generated_file(arg_path, classes)

    structured_classes = json.dumps({"contents": classes_as_dictionary}, indent=4)
...
def main():
    ...
    stack2 = search_file(arg_path + "/src/enemy_example.h")

    dic = {"file_contents": stack2[0].get_as_dictionary()}

    print(dic)

    s = json.dumps(dic, indent=4)
    f = open(arg_path + "/parsed_file.json", "w")
...
def search_folders_for_ufo_classes(_classes, _working_directory, _local_path):
    for directory in os.listdir(_working_directory + _local_path):
        ...
        if os.path.isdir(_working_directory + "/" + _local_path + "/" + directory):
            search_folders_for_ufo_classes(
                _classes, _working_directory, _local_path + "/" + directory
            )
        if os.path.isfile(_working_directory + "/" + _local_path + "/" + directory):
            ...
            if extension in [".ufo.hpp", ".ufo.h"]:
                ...
                stack[-1].search(pending_macro, _classes, "")

    # for path,folder,files in os.walk(_working_directory):
    #    if(path.) print("path,folder,files=",path,folder,files)


# main()

start_search_directories_for_ufo_classes()
